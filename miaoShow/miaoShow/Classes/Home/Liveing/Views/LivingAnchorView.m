...
@interface LivingAnchorView()
@property (weak, nonatomic) IBOutlet UIView *anchorView;
@property (weak, nonatomic) IBOutlet UIImageView *headImageView;
@property (weak, nonatomic) IBOutlet UILabel *nameLabel;//主播昵称
@property (weak, nonatomic) IBOutlet UILabel *peopleLabel;//观看人数
@property (weak, nonatomic) IBOutlet UIScrollView *peoplesScrollView;//观众
@property (weak, nonatomic) IBOutlet UIButton *giftView;
@property (weak, nonatomic) IBOutlet UILabel *anchorID;//主播喵播号
@property (strong, nonatomic) NSTimer *timer;
@property (strong, nonatomic) NSArray *audienceUsers; //观众


@end
@implementation LivingAnchorView
- (NSArray *)audienceUsers
{
    if (!_audienceUsers) {  //暂时用这个 数据  后面再换成实际网络获取到的数据
        NSArray *array = [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"user.plist" ofType:nil]];
        _audienceUsers = [User mj_objectArrayWithKeyValuesArray:array];
    }
    return _audienceUsers;
}
...
#pragma  mark  观众设置
- (void)setupAudienceUsers
{
    self.peoplesScrollView.contentSize = CGSizeMake((self.peoplesScrollView.height + DefaultMargin) * self.audienceUsers.count + DefaultMargin, 0);
    CGFloat width = self.peoplesScrollView.height - 10;
    CGFloat x = 0;
    for (int i = 0; i<self.audienceUsers.count; i++) {
        ...
        User *user = self.audienceUsers[i];
...
-(void)clickaudienceUsers:(UITapGestureRecognizer *)tapGes{
    if (tapGes.view == self.headImageView) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:kNotifyClickUser object:nil userInfo:@{@"user" : self.audienceUsers[tapGes.view.tag]}];
    }
...
- (void)setLive:(LiveListModel *)live
{
    ...
    self.anchorID.text =[NSString stringWithFormat:@"喵播号:%@",live.useridx];
    ...
    [self.headImageView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(clickaudienceUsers:)]];
